...
Alternative (non recommandée par défaut) : texte superposé seulement si contraste et espace négatif garantis.
...
| Contenu textuel provisoire | Sur-titre `FLIPPIN’ MAPLE` ; H1 `NO FIXED LINE.` (emplacement de travail) ; appui 1–2 phrases à définir |
| CTA / destinations | `Découvrir la collection` → boutique / collection ; CTA secondaire optionnel seulement si action distincte |
| Placeholder | `[PLACEHOLDER — IMAGE HERO CAMPAGNE PRINCIPALE]` — détail §16 |
| Responsive | Split bureau ; stack 4:5 mobile ; CTA tactile pleine largeur |
...
| CTA | Libellé explicite « Découvrir la collection » |
| Focus | Visible sur le CTA |
| Contraste | Texte lisible sur zone texte ou fallback sombre |
...
| Retrait / fallback | Catégories non validées → retrait ou destinations réelles seulement |
...
| Contenu textuel provisoire | Titre : À définir lors de la validation de la voix de marque ; phrase à définir |
| CTA / destinations | Bouton « S’inscrire » → soumission ; lien Confidentialité |
| Placeholder / actif | Aucun actif requis (motif optionnel très discret si produit) |
...
| Ouvert | Textes finaux ; dépendance `VOICE.md` pour formulations ; sélecteur de langue = dépendance technique future |
